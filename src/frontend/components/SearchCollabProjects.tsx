...
export const SearchCollabProjects = ({
  allProjects,
  projectSlug,
  collabType,
  setValue,
  ...
  const [selectedProjects, setSelectedProjects] = useState<Project[]>([]); // Changed this to an array
  ...
  const handleSelectProject = (project: Project) => {
    setSelectedProjects((prevSelectedProjects) => {
      ...
      const isAlreadySelected = prevSelectedProjects.find(
        (selectedProject) => selectedProject.id === project.id
      );

      if (isAlreadySelected) {
        // Remove project from the selected projects array
        newSelectedProjects = prevSelectedProjects.filter(
          (selectedProject) => selectedProject.id !== project.id
        );
      } else {
        // Add project to the selected projects array
        newSelectedProjects = [...prevSelectedProjects, project];
      }

      // Now use the newSelectedProjects array to set the value
      setValue(newSelectedProjects.map((project) => project.id));

      // Return the new state for selectedProjects
        ...
        >
          <span className="text-sm text-gray-400">
            {selectedProjects?.length > 0
              ? selectedProjects.map((x) => x.name).join(",")
              : "Select one or more"}
          </span>
