    ...
    <div className="space-y-4 sm:space-y-3 p-3 bg-dark-600 rounded-lg">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Placeholder"
          className={`form-input w-full rounded-md text-sm bg-gray-800 border-gray-700 text-gray-400 ${
            errors?.holderRules?.[ruleIndex]?.discordRoleRule.guildName
              ? "border-red-500"
              : ""
          }`}
        />
...
// get projects by search query

export function CreateCollabPublic({
  giveaway,
  mode,
}: {
  ...
  const onSubmit = (data: GiveawayInput) => {
    ...
    if (!myProject?.discordGuild && collabType === CollabType.RECEIVE_SPOTS) {
      setConfirmOpen(true);
    } else {
      submitForm(data);
    }
  ...
  const { data: ownedProjects, isLoading } = useOwnedProjects();
  const collabType = watch("collabType");

  const projectSlugWatch = watch("projectSlug");

  const { data: myProject } = useProject({
    slug: projectSlugWatch as string,
  });

  useEffect(() => {
    if (projectSlugWatch) {
      const collabProject = ownedProjects?.find(
        (project) => project.slug === projectSlugWatch
      );

      if (collabProject) {
        setValue(
          "name",
          `${project?.name ?? ""} X ${collabProject?.name ?? ""}`
        );
      }
    }
  }, [projectSlugWatch]);
  ...
  useEffect(() => {
    if (collabType === CollabType.GIVE_SPOTS) {
      if ((myProject?.defaultRules?.length ?? 0) > 0) {
        setValue("rules", myProject?.defaultRules ?? []);
      } else {
        setValue("rules", []);
      }
  ...
  if (!isLoading && !ownedProjects?.length) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">
            You need to create a project before you can create a giveaway
          </p>
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="space-y-4 sm:space-y-3 mb-12">
          {/* Select project */}
          <div>
            <div className="flex justify-between items-center">
              <label className="block text-sm font-medium mb-2 dark:text-white">
            ...
            <select
              ...
              className={`form-select w-full rounded-md text-sm bg-gray-800 border-gray-700 text-gray-400 ${
                errors.collabProjectId ? "border-red-500" : ""
              }`}
            >
            ...
            {errors.collabProjectId && (
              <p className="text-red-500 text-xs mt-1">
                {errors.collabProjectId.message}
              </p>
            )}
          </div>
          <div>
            ...
            >
              ...
              {project?.phase === ProjectPhase.POSTMINT ? (
                <option value={CollabType.GIVE_SPOTS}>Give Spots</option>
              ) : (
                <>
                ...
                <div className="w-full">
                  ...
                  >
                    ...
                    {Object.values(GiveawayRuleType)
                      .filter(
                        (x) =>
                          x !== GiveawayRuleType.OWN_NFT &&
                          x !== GiveawayRuleType.MINIMUM_BALANCE
                      )
                      .map((type) => (
                        <option key={type} value={type}>
                          <>
          ...
          <div>
            {myProject?.allowlist?.type === AllowlistType.DISCORD_ROLE &&
              collabType === "GIVE_SPOTS" &&
              myProject?.phase === "PREMINT" && (
                <>
                  <label
                    htmlFor="name"
                  ...
                  <div className="">
                    ...
                    >
                      <option value={undefined}>Select One</option>
                      {myProject.allowlist?.roles?.map((role) => (
          ...
          {myProject?.allowlist?.type === AllowlistType.DTC && (
            <>
              <div>
                <div className="flex gap-2 items-center">
                  <label className="block text-sm font-medium mb-2 text-green-400">
          ...
          >
            {mode === "edit" ? "Update Giveaway" : "Send Collab Request"}
          </button>
        </div>
      </form>
