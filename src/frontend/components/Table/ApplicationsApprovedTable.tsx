...
import { useProjectApprovedApplications } from "@/frontend/hooks/useProjectApprovedApplications";

export const ApplicationsApprovedTable = ({
  project,
}: {
  project: Project;
}) => {
  ...
  const { data: ret, isLoading } = useProjectApprovedApplications({
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 items-center justify-between sm:w-fit w-full">
        <span className="text-neutral-500">{total ?? 0} approved</span>
      </div>
      {applications && (
        <Table
          data={applications}
