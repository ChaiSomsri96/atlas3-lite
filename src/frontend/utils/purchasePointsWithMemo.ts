...
export const purchasePointsWithMemo = async ({
  connection,
  wallet,
  to,
  mint,
  ...
  if (!points) throw new Error("Enter points to purchase");
  ...
  const amountBN = points * Math.pow(10, mintInfo.decimals);
  ...
  if (fromTokenAccount.amount < amountBN) {
    throw new Error("You do not have enough USDC for this purchase.");
  }
