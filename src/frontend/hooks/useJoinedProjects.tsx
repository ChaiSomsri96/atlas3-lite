...
export type JoinedProjectsResp = {
...
export const useJoinedProjects = ({
  ...
  return useQuery<JoinedProjectsResp>(
    [
      PROJECT_KEY,
      "joinedProjects",
      session?.user?.id,
    ...
    async () => {
      const response = await fetch(
        `/api/me/joined-projects?sortOption=${sortOption}&filterOptions=${filterOptions}&page=${page}&pageLength=${pageLength}`
      );
